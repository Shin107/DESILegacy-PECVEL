...
class PhotoZTabNetPipeline:
    ...
    def load_astronomical_data(self):
        ...
        north_DR1 = Table(fitsio.FITS('/Storage/animesh/PECVEL/LEGACY+DR1/TABLE_DR1_north_v1_sep_10_all_features.fits')[1].read())
        south_DR1 = Table(fitsio.FITS('/Storage/animesh/PECVEL/LEGACY+DR1/TABLE_DR1_south_v1_sep_10_all_features.fits')[1].read())
        DR1 = vstack([north_DR1, south_DR1])

        # Load Legacy data
        north_legacy = Table(fitsio.FITS('/Storage/animesh/PECVEL/LEGACY+DR1/TABLE_legacy_north_v1_sep_10_all_features.fits')[1].read())
        south_legacy = Table(fitsio.FITS('/Storage/animesh/PECVEL/LEGACY+DR1/TABLE_legacy_south_v1_sep_10_all_features.fits')[1].read())
...
def main():
    ...
    if args.mode == 'train':
        ...
        pipeline.save_model(args.model_path if args.model_path else "tabnet_sep_10_huber.zip")
