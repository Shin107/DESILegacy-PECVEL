...
parser.add_argument('-d', '--directory', choices=['north', 'south'], default='south',
...
parser.add_argument('--error_matching', type=float, default=0.0,
                    help='Error matching value to apply (default: 0.0, )')
parser.add_argument('-t','--test', action='store_true', help='Run on a test subset of the data')
parser.add_argument('--file_suffix', type=str, default='', help='Suffix to add to output file name (default: empty string)')
args = parser.parse_args()
directory= args.directory
zpt = args.zeropoint_shift


err_match  = args.error_matching
end_name = args.file_suffix
process = psutil.Process(os.getpid())

print(f'Error matching value: {err_match} err_match > 0: {err_match > 0.0} err_match == 1: {err_match == 1} err_match == 2: {err_match == 2}')
def read_stat_files():
    ...
    return north_stats, south_stats
...
def process_file_pair(paths):
    ...
    # try:
       ...
       'FRACIN_Z','GAIA_PHOT_G_MEAN_MAG','NOBS_G','NOBS_R','NOBS_Z','FIBERFLUX_G', 'FIBERFLUX_R', 'FIBERFLUX_Z','FLUX_W1','FLUX_W2','FLUX_W3','FLUX_W4','MW_TRANSMISSION_W1','MW_TRANSMISSION_W2','MW_TRANSMISSION_W3','MW_TRANSMISSION_W4','PSFDEPTH_G','PSFDEPTH_R','PSFDEPTH_Z','FLUX_IVAR_G','FLUX_IVAR_R','FLUX_IVAR_Z','FLUX_IVAR_W1','FLUX_IVAR_W2','FLUX_IVAR_W3','PSFSIZE_G','PSFSIZE_R','PSFSIZE_Z','EBV',]
        ...
        for i in ['G','R','Z','W1']:
            ...
            print(f'flux_original: {flux_original[:5]}')
            if directory == 'south' and 'W1' not in i:
                ...
                north_stats, south_stats = read_stat_files()
                ...
                fraction_in_range = np.sum(in_range) / len(flux)
                if err_match > 0.0:
                    ...
                    if err_match ==1: 
                        ...
                        flux[in_range] += np.random.normal(0, np.sqrt(sigma_north**2 - sigma_south**2))
                    
                    elif err_match ==2:
                        print('\n\n\n\n')
                        medians_south = {'G': 0.11035499453639724, 'R': 3.9777341885891007, 'Z': 0.3229655783979233}
                        medians_north = {'G': 0.8500512642519815, 'R': 18.694841366587504, 'Z': 24.08291971182623}
                        sigma_south = medians_south[i]
                        sigma_north = medians_north[i]
                        flux += np.random.normal(0, np.sqrt(sigma_north**2 - sigma_south**2), size=flux.shape)
                        print(np.random.normal(0, np.sqrt(sigma_north**2 - sigma_south**2), size=flux.shape))
                    else: 
                        import sys
                        print(f"Invalid error matching value: {err_match}. Use 0 for no matching, 1 for spline-based matching, or 2 for median-based matching.")
                        sys.exit(1)
   
            print(f'flux_original: {flux_original[:5]}, flux_corrected: {flux[:5]} for band {i} in file {fits_path}')
            #assert flux_original!= flux, "Flux values did not change after error correction."
            ...
            MAG_OG[i] = 22.5 - 2.5 * (np.log10(flux_original / trans, where=(frac > 0)))
        for i in ['G','R','Z']:
        ...
        print(f'MAG R  after degradation:{MAG["R"][:5]} and before degradation: {MAG_OG["R"][:5]} for file {fits_path}')
        # Merge and save
        # final = {}
    ...
    #print('\n\n ',len(final_table),'\n\n')
        if err_match > 0.0:
                return final_table, dct, fraction_in_range
        else:
                return final_table, dct, None
...
def main():
    ...
    output_file = f"/user/animesh.sah/FP_CUTS/{directory}_cuts_degraded_{end_name}.fits"
    ...
    table, cut_counts, fraction_in_range = zip(*results)
    merged = {}
    ...
    import pickle
    with open(f"south_dict_{end_name}.pkl", "wb") as f:
    ...
    df.to_parquet(f"summary_cuts_south_degraded_{end_name}.parquet")
    print(df)
    
    # Aggregate numeric fields only (cuts and cumulative are arrays, others are scalars)
    for d in cut_counts:
        for k, v in d.items():
            if k in ['cuts', 'cumulative']:
                # Sum numpy arrays element-wise
                summary[k] = summary.get(k, 0) + v
            elif k in ['total', 'masked', 'final_table_length']:
                # Sum scalar values
                summary[k] = summary.get(k, 0) + v
            # Skip 'name' as it's a string identifier

    # Print summary
    print("Summary of selection cuts:")
    for key, value in summary.items():
        print(f"  {key}: {value}")
    print(f"Fraction of flux values in valid range for error matching: {fraction_in_range}")
    np.save(f"fraction_in_range_{end_name}.npy", np.array(fraction_in_range))
